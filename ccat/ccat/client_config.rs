extern {
    ...
    fn catAnetGetHost(
        err : *mut u8, host : *mut u8, ipbuf_len : usize
    ) -> i32;
...
#[no_mangle]
pub static mut g_config
    : _CatClientInnerConfig
    = _CatClientInnerConfig {
          appkey: 0 as (*mut u8),
...
#[no_mangle]
pub unsafe extern fn catChecktPtrWithName(
    mut ptr : *mut ::std::os::raw::c_void, mut ptrName : *mut u8
) {
...
#[no_mangle]
pub unsafe extern fn isCatEnabled() -> i32 { g_cat_enabledFlag }
...
#[no_mangle]
pub unsafe extern fn parseCatClientConfig(mut f1 : *mut ezxml) -> i32 {
    ...
    'loop1: loop {
        ...
        'loop6: loop {
            ...
            if !(0i32 as (*mut ::std::os::raw::c_void) as (*const u8) == ip || *ip.offset(
                                                                                    0isize
                                                                                ) as (i32) == b'\0' as (i32)) {
                if serverIndex == 0i32 {
    ...
    ezxml_free(f1);
    if serverIndex <= 0i32 { -1i32 } else { 0i32 }
}

unsafe extern fn getCatClientConfig(
...
#[no_mangle]
pub unsafe extern fn loadCatClientConfig(
    mut filename : *const u8
) -> i32 {
...
#[no_mangle]
pub unsafe extern fn initCatClientConfig(
    ...
    if catAnetGetHost(
        ...
        g_config.selfHost = catsdscpy(
                                g_config.selfHost,
                                (*b"CUnknownHost\0").as_ptr()
                            );
    }
    CLogLogWithLocation(
        0x2u16,
    ...
    g_config.serverAddresses = malloc(
                                   (g_config.serverNum as (usize)).wrapping_mul(
    ...
    'loop3: loop {
        ...
        *g_config.serverAddresses.offset(i as (isize)) = catsdsnew(
                                                             (*b"\0").as_ptr()
                                                         );
        i = i + 1;
    }
...
#[no_mangle]
pub unsafe extern fn clearCatClientConfig() {
