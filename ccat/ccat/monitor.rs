...
pub struct __darwin_pthread_handler_rec {
    pub __routine : unsafe extern fn(*mut ::std::os::raw::c_void),
...
static mut g_cat_monitorHandle
    : *mut _opaque_pthread_t
    = 0 as (*mut _opaque_pthread_t);
...
unsafe extern fn catMonitorFun(
    ...
    'loop1: loop {
        ...
        if runCount.wrapping_rem(
               60usize
           ) == 1usize && (g_config.enableHeartbeat != 0) {
            logEvent(
            ...
            if strcmp(
                   g_client_info.language as (*const u8),
                   (*b"C\0").as_ptr()
               ) != 0i32 {
...
#[no_mangle]
pub unsafe extern fn initCatMonitorThread() {
...
#[no_mangle]
pub unsafe extern fn clearCatMonitor() {
